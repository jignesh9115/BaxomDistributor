...
public class FileUriUtils {
    ...
    private static FileUriUtils instance;
    ...
    public static FileUriUtils getInstance() {
        if (instance == null) {
            synchronized (FileUriUtils.class) {
                if (instance == null) {
                    instance = new FileUriUtils();
                }
            }
        }
        return instance;
    ...
    public String getURIForExternalstorageDoc(Uri uri) {
        String documentId = DocumentsContract.getDocumentId(uri);
        String[] idArr = documentId.split(":");
        if (idArr.length == 2) {
            String type = idArr[0];
            ...
            if ("primary".equalsIgnoreCase(type)) {
                return mContext.getExternalFilesDir("").getAbsolutePath()  + "/" + realDocId;
            }
        }
        return null;
    }
